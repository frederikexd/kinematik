...
## 20-second tour (the one idea)

KinematiK is **one car, not eight tools.** Every subsystem declares what it weighs, draws, rejects and provides into a single **Integration ledger**, and that one source feeds the 3D model, the lap sim, the heatmap and the cost BOM — declare a number once and it propagates everywhere, so the eight "we're ~12 kg" estimates can't quietly sum to 18 kg over the number suspension tuned to.

Three moves are all you need to start:

1. **Declare your interface.** In **Integration**, fill what your subteam owns (mass, CG, torque, heat, current, downforce…) and untick *estimate* once a number is real. Everything downstream now uses it.
2. **Watch it ripple.** The moment you save, KinematiK walks your change through its coupling graph and tells you which *other* subsystems' risk just moved — and whether that's a **measured** solver result, a **coupled** model, or an **engineering judgement**. No green light ever pretends to be more certain than the data behind it.
3. **Clear the cut.** Before a part goes to manufacture, run the **manufacturing-release gate**: a literal go / no-go that blocks any part still resting on an estimate or an unconfirmed load. When the gate doesn't hard-block, you can **record a sign-off** — tick a box confirming you checked the approach and the loading edges with your lead, name that lead, and it's written (timestamped) into the handover decision log, so it lands in the PDF/Markdown/JSON export instead of a file that gets lost. One manufacturing shot, one honest checklist.

> ⚠ **Always confirm with your subsystem lead before cutting.** The gate is a safety net, not a sign-off. Its coupling edges are a sensible starting set, not gospel for *your* car — a real load path may be missing, and a "clear" verdict only means the numbers you declared are self-consistent. Have the lead sanity-check the approach and the edges loading each part before committing anything to manufacture.

Everything else — kinematics, aero, powertrain, lap time, DFMEA, the 23 tabs below — hangs off that spine.

---

## 30-second tour (every tab)

Twenty-three tabs, front to back. Each takes the live geometry + setup and answers one question; nothing is retyped between them — mass/CG, peak currents, ride heights, downforce and the tire all flow from one source of truth (the **Integration** ledger), so declaring a number once propagates everywhere.

**Start by finding yourself on the car.** Before the tabs, a **3D role picker** greets you: a live, drag-to-rotate Formula Student car next to a subteam selector. Pick your subteam(s) and the parts that team physically owns **light up on the car** while everything else dims back — Aerodynamics glows the wings, Brakes the discs, Suspension the corners, Electrics the HV battery box, cooling sidepods and electronics, and so on — each tagged with a floating label in your subteam's own colour (matched to the team's Discord channel hearts). Cooling is a shared concern, so its sidepod/radiator bodies light for both Powertrain and Electrics. Blend as many subteams as you wear; their tabs union up front and the rest tuck under **⋯ More tools**. It's pure visual feedback — the dropdown drives selection, the car just shows you what's yours — so a new member sees the metal they're responsible for before ever opening a tab.

**Find your subteam fast:** Suspension/Dynamics → Kinematics, Roll & Load Transfer, Grip Balance, Compliance, Tire & Grip, Setup Optimiser, GGV, Transient · Aerodynamics → Aerodynamics · Powertrain/Drivetrain → EV Powertrain, Lap Time, DFMEA · Electrics → Accumulator, Lap Time (electrical feasibility check), Electronics (PCB), Tractive Safety, DFMEA (+ the electrics layer of the Heatmap) · Cooling → DFMEA + sidepod/radiator bodies + the heat layer of the Heatmap (shared with Powertrain & Electrics) · Data Acquisition → Electronics (PCB), Data-acq in Integration · Brakes → Brakes · Chassis/Frame → 3D Model, Team Fit, Integration, Weight & Handover · Cost/Business → Cost & BOM · Everyone → 3D Model, Integration, **Registry**, Lead Notes, Validation.
...
4. **3D MODEL** — the live 3D view, three views via a toggle. **Subsystems**: pick a subsystem (suspension, aero, powertrain, cooling, electrics, brakes, chassis, data-acq), then an individual **part** within it — wishbone, upright, tie rod, wing, radiator, accumulator, brake disc, monocoque, etc. — to zoom the camera onto that one part, spotlit in whole-car context; the suspension also exposes its **inner pickup points** (wishbone pickups, ball joints, tie-rod ends), and anything that exists at every wheel gets a **corner row (FL / FR / RL / RR)** to drill to a single corner. You can also click any part or corner directly on the model. **Full car**: a true Formula Student EV assembled from every subsystem's current declaration (nosecone, survival cell, front/rear wings, roll hoops, driver + helmet, sidepods sized by cooling airflow, traction motor + inverter sized by power, accumulator, brake discs sized by torque, CG marker from the mass roll-up) — editing a hardpoint, spring rate, downforce or battery mass anywhere visibly moves the matching body, and clicking a part auto-zooms onto it. **Heatmap**: shades the whole car by **heat rejection, electrical draw, or mass** — cool = light load, hot = heavy load — so the powertrain↔cooling↔electrics coupling is legible at a glance; bodies that exceed what the car can carry (heat beyond cooling airflow, draw beyond LV supply, mass over budget) are outlined in magenta, with a ranked contribution table and the relevant integration warnings underneath. Colours and warnings come from the Integration ledger, so the picture and the numbers never disagree. **Import from anywhere**: every subsystem tab (Suspension, Aerodynamics, Powertrain, Cooling, Electrics, Brakes, Data Acquisition) now has its own **Import CAD / sketch** drop zone — drop a STEP/STL/OBJ/GLB and the real geometry appears on the full-car 3D model immediately, tagged to that subteam so it lights up in the subteam's colour and dims with the rest under the role picker. Import under Brakes, see it on the full car; it's the same shared parts list both directions, so subteams build up the assembled car without ever leaving their own tab.
5. **AERODYNAMICS** — the aero subteam's workspace, three views. **Downforce & ground effect**: live force-vs-speed (∝ V²) and downforce-vs-ride-height curves, so ground effect is visible as you lower the car. **Wing & diffuser sizing**: size the package to a downforce target, see the drag it costs, the front/rear split it lands, and how much the floor/diffuser carries vs the wings — then push it into the car's declared downforce so the 3D wings, lap sim and heatmap all reflect it. **Aero map**: the ride-height × yaw attitude sweep (downforce / drag / balance) the lap sim queries through a lap. Driven by the in-house analytic aero model for instant trends, with an honest provenance note and the upgrade path to the shipped 3D source-panel potential-flow solver (real ground effect from your STL), the Virtual Wind Tunnel, pressure-tap correlation, and the ANSYS Fluent verification deck.
...
7. **ACCUMULATOR** — the HV battery-pack designer. Pick a cell (presets for common 18650/21700 cells, or custom), set the series/parallel topology, and it sizes the pack — voltage, energy, capacity, cell count, mass, per-cell current, C-rate, deliverable power, and I²R heat — then runs the hard FSAE-EV rules checks: tractive system < 600 V, tractive power ≤ 80 kW, per-cell current within the cell's rating, and the < 120 V / < 6 MJ isolatable-segment requirement (EV1 / EV3.3.3), suggesting the minimum compliant segment count. It draws the pack grid coloured by segment, and a button declares the pack (voltage, mass incl. housing, heat) into the Integration ledger so the heatmap, mass roll-up and electrical checks all use it; the layout also feeds KinematiK's per-cell transient thermal model. The declared pack flows directly into the **electrical feasibility gate** in the **LAP TIME** tab — upload a `SpeedVsTime` sheet (column A: time in seconds, column B: speed in mph) and the checker runs the speed profile against your fuse rating, per-cell current limit and usable energy in one pass, reporting peak pack current, peak cell current, total energy draw, and the fuse-limited speed ceiling; all three hard limits must pass before the lap is electrically feasible.
8. **BRAKES** — the brake system designer, built around the FSAE brake test (lock all four wheels, stop straight). **Bias & lock-up**: from the live car's mass, CG height and wheelbase it computes the forward load transfer under braking, the ideal front/rear bias vs deceleration, which axle locks first (front-first is stable, rear-first spins the car), the max decel before lock, and the torque needed per corner to lock the tyres — with a brake-bias diagram. **Hydraulic sizing**: sizes the pedal-ratio → master-cylinder → caliper → pad → rotor chain to deliver that lock-up torque, reporting line pressure, clamp force, torque made vs needed, and the pedal force a driver actually needs — then declares the brake torque/corner into the Integration ledger so the 3D disc size and heatmap reflect it. **Bolt & bracket FoS**: the view that retires the team's most-repeated pain point — *"all the stress in my Ansys is from the torque I put in the bolts, and I don't know how much torque we actually need."* It computes the bolted joint from first principles (VDI 2230 / Shigley): the **preload from assembly torque** (F = T/(K·d), with the nut-factor K exposed because lubrication is the biggest preload scatter source), the **bolt working stress vs proof** (so an over-torque that yields the bolt at install — the red ring the FEA keeps showing — is caught here, not in the mesh), the **joint-separation margin** (the external load at which the clamp gaps open and the bolt sees the full raw spike), and the **head-bearing crush check** against the softer clamped material (the case a hardened washer fixes). The external pull-off load can be **seeded straight from the live lock-up torque** the Bias view found, reacted through the caliper-mount bolts, so the whole chain stays connected instead of being re-typed. A **torque sweep** plots bolt stress and separation margin against assembly torque, so the safe window *between* "too loose" and "yields at install" is the torque spec — read off, not guessed. A second mode screens any **caliper bracket or pedal tab against the chassis-team FoS ≥ 1.5-on-yield rule** in five seconds, reporting which failure mode governs (root bending almost always wins — which is exactly why a plate that *looks* fine lights up at the root in FEA) with a per-mode FoS bar. Both are honest closed-form hand calcs: the prying amplification and weld-toe HAZ are flagged as the FEA-shaped parts, so a green light here *earns* the Ansys run rather than replacing it — the sim becomes a **verification of a number you already trust** instead of the place you discover it. **Rotor thermal**: a fast 0-D lumped-mass rotor heat model (the same energy-balance approach as the tyre thermal channel, with braking power as the input) that answers single-stop peak temperature, endurance equilibrium temperature, and the **mass-vs-peak-temp frontier** across rotor diameter — so a team optimising rotors in Ansys can sweep dozens of geometries here in milliseconds and only mesh the 6 or fewer survivors. It is explicitly **not** an FEA (no temperature field, coning, vane flow or stress) and flags every output as synthesized until calibrated: right shape, representative magnitudes, for triaging which geometries deserve a full FEA run. The endurance number can be driven straight from a **lap sim** — one click runs the quasi-steady lap, sums the braking energy the velocity trace implies, and fills in the true average power per front rotor and mean braking speed, so the equilibrium temperature reflects a real lap rather than a typed guess. It goes well past a single number: a **two-node transient** (rotor + pad masses integrated through time) runs a repeated-stop **fade test** and shows the rotor and pad climbing run-to-run — the thermal accumulation a single-stop figure misses — against the material's service ceiling; a **material library** (grey iron, steel, carbon-carbon, Al-MMC, each with real density/cp/conductivity/temp-limit) compares mass vs. peak temp on the same test in one table; a first-principles **rotating-disc convection** model (rotational + cross-flow Nusselt) replaces the hand-set coefficient and responds physically to rotor size, speed and a **brake-duct airflow gain**; and an inverse solver answers **"what cooling duct do I need"** to hold a target temperature, turning the thermal model directly into a duct spec. The headline is a **rotor optimiser**: the real pain in FEA-based rotor design isn't the single solve, it's the *loop* — re-parametrise, re-mesh, re-apply boundary conditions, re-run, times forty. This searches diameter × thickness × vent × material for the **lightest rotor that still survives the fade test**, evaluates the whole space in about a second over the transient model, and returns the winner, the mass-vs-temp Pareto front, and a short-list of up to 6 knee-point geometries to mesh — so the lead targets only the Ansys jobs that matter instead of forty on the whole sweep. It owns the optimisation loop; Ansys verifies the winners (hot-spot, coning, stress). **Export to CAD**: once the short-list is in, a one-click **DXF export** (R12, one file per rotor) writes the half-section profile as a closed polyline — hub bore, outer radius, thickness, vent depth marker, and a descriptive title block — ready to revolve into a 3-D solid in SolidWorks, Fusion 360, FreeCAD, or AutoCAD with zero manual geometry entry. Coordinates and the title block switch automatically between **millimetres and inches** to match the app’s active unit system (`$INSUNITS` set accordingly so the file opens at the correct scale without any manual override). A companion **config script** (`.py`) records material, all dimensional parameters, and predicted mass/peak-temp in the same unit system — with metric equivalents in comments when imperial is selected — for use in CAD macros or as a traceable design-intent record alongside the DXF.
9. **COST & BOM** — the scored FSAE Cost & Manufacturing event, without the blank-sheet pain. It auto-seeds a starter Bill of Materials from the masses every subsystem already declared in Integration, pre-fills standardized catalog costs (material $/kg by class, plus process rates for holes/cuts/bends/labour and machining), and lets you edit every row inline. It rolls up cost per commodity and against your target, charts cost-by-commodity and make-vs-buy, and exports the BOM as CSV — following the FSAE hierarchy (System/Commodity → Part → Material/Process/Fastener) so material and manufacturing choices show up live as dollars.
10. **COMPLIANCE (FLEX)** — the links aren't rigid: pick a lateral g and tube size (optionally chassis-tab stiffness in series, or a condensed FEA flex body) and read the deflected toe/camber — the ADAMS-Flex-style compliance most free tools assume away. The same structural layer also carries an analytic **bolted-joint check** (`bolted_joint.py`): preload from torque, the joint load factor, and whether a braking load pries the pedal-box base off its tabs (VDI 2230 / Shigley — separation, bolt stress, washer-face crush).
11. **TEAM FIT** — the cross-team CAD check: any subteam loads the shared chassis as reference, then drops in their part (caliper, radiator, battery box, wing mount, ECU tray) and gets the same collision / tight / clear verdict suspension gets — catch interference before the first cut.
12. **WEIGHT & HANDOVER** — the mass + CG ledger (the single source the load-transfer model reads), a **searchable design-decision log** (log a decision with rationale/author/part, then find it by free-text, team, tag or part), and a one-click **handover export** in three formats — Markdown, raw project `.json`, and a formatted **PDF** — so design decisions and their evidence survive a roster change. Unresolved lead notes are carried into the export as loose interface ends.
13. **LEAD NOTES** — cross-team notes between subsystem leads, addressed to a team (or all), each with an open/resolved status and `requests action` / `urgent` flags, so a note is a tracked action item rather than a Discord message that scrolls away. Notes persist next to the work in the project (`project.json`, or Supabase when configured) and carry into the PDF handover as unresolved interface items. **Live notification:** when any lead posts a note, every other open session is notified within ~10 s — a toast pops on whatever tab they're viewing and an unread badge appears on the LEAD NOTES tab.
14. **TIRE & GRIP** — the Pacejka MF5.2 lateral model: load sensitivity, camber response, peak-µ / optimal-camber search, combined-slip friction ellipse, relaxation length, and the **thermal channel** (warm-up / working-range / pressure). Load your tire either way — upload an already-fitted `my_tire.json`, **or drop a raw TTC cornering `.mat`/`.csv` straight from the rig** and the app cleans it, fits the Magic Formula in-browser (R²/RMSE reported), applies it live, and hands you back the private JSON to keep. Everything downstream then uses *your* tire; uncalibrated channels stay flagged, never faked.
15. **SETUP OPTIMISER** — sweeps the physical levers (spring rates, ARB, ride height …) through the motion ratio and **ranks setups by lap delta**; sensitivity()/optimise() never return a setup worse than the start.
16. **LAP TIME** — quasi-steady-state point-mass lap with a configurable powertrain & aero block: the three timed FSAE events out of the box — skidpad (one timed circle), 75 m accel (standing-start integration) and autocross — using aero + a real motor torque map; **track from GPS/cone coordinates** and a curvature-optimal **racing line** with the seconds it gains. Also home to the **electrical feasibility gate** (visible to the Electrics subteam): upload the electrics lead's Excel workbook (needs `Battery Pack Calcs`, `ElecPropulsion`, and `SpeedVsTime` sheets) and it checks the speed profile against the fuse rating, per-cell current limit and usable pack energy — flagging whether the lap is electrically achievable before anyone finds out on track.
17. **GGV DIAGRAM** — the accel-lateral-velocity envelope the lap sim rides, drivable from the live vehicle + the same powertrain, with a one-click check that GGV and lap sim agree.
18. **TRANSIENT** — the unsteady half of the lap: an explicit high-frequency time-step solver for turn-in and pitch built on relaxation length + the damper model, the thing QSS assumes away. Run a manoeuvre, export the time history as `.csv`, and save/restore the whole working project as `.json`.
19. **VALIDATION** — earn trust by matching data. Skidpad g / circle time, 75 m accel, and a GPS **speed-trace** RMSE/bias/R²; the **Virtual Tunnel Solver** (sweep the physical aero map → **write the solver case files** → run the matched k-omega SST points through **Star-CCM+, TS-Auto *and* OpenFOAM at once** → fuse them into one cross-code **consensus**, with the **inter-code spread** as the confidence signal → point-by-point C_l/C_d/balance calibration); **surface pressure taps** (raw volts → C_p on the wing → stall detection → RMSE vs CFD); and the **live acquisition front end** — a Virtual Instrument off the force balance + Scanivalve/Chell scanners. No number is tuned to fit, no single code is just trusted, and a solver that can't run is an honest hole, never a fabricated number.
20. **INTEGRATION** — the live cross-subsystem surface, in several views: a **cross-subsystem ledger** (the one place mass, CG, peak currents, heat, power and downforce are declared, against shared car-level budgets/limits — and the source the 3D model, heatmap, lap sim and cost BOM all read), **subsystem ↔ chassis CAD fit**, and **mount-point clash** — move a mount and the clash verdict plus mass/CG propagate in one call. It also **feeds the as-built numbers back into the physics**, keeps a **pending-change log**, and exports a Markdown **interface report**. Persists with the project. **Cross-discipline risk propagation:** when any sub-team saves an interface edit, KinematiK walks the change through a coupling graph and shows, unprompted, which *other* subsystems' risk just moved — bump the motor torque and you immediately see it load the upright and heat the cooling loop. Every effect names both disciplines (so the risk has an owner) and carries an honest confidence tag — **measured** (a KinematiK solver actually ran), **coupled** (a modelled physical edge), or **judgement** (engineering judgement, no backing physics) — and a measured edge is *demoted* if the data behind it is still an estimate, so a green board never overstates what's known. Those shifts roll straight onto the team's live **DFMEA** rows in their own wording, as suggested Occurrence nudges with RPN recomputed by the DFMEA engine. **Manufacturing-release gate:** for a team with one manufacturing shot, the season-ending failure is a part cut to a number that was still an estimate, or designed against a load that was only an engineering guess. Pick the subsystem about to be made and the gate gives a literal **go / no-go** — it blocks if the interface is still flagged *estimate*, if there's an open integration conflict, or if a load feeding the part comes from an unconfirmed source; warns on judgement-only loads or a missing rationale; and clears only when the interface is confirmed and the loads are solver-backed. Metal you cut wrong, you cut once — so the tool enforces the check instead of trusting that someone remembered to ask. **This gate is a safety net, not a sign-off:** its coupling edges are a sensible default, not a guarantee for your specific car, so always confirm with your subsystem lead before cutting — the lead should sanity-check both the approach and the edges loading each part, and a "clear" verdict only means the declared numbers are self-consistent. When the gate doesn't hard-block, it offers a **sign-off interlock**: a required checkbox confirming the approach and the loading edges were checked with the lead, plus the lead's name, which writes a timestamped entry into the handover decision log (so it flows into the PDF/Markdown/JSON export — a durable record, not a downloaded file that gets lost). A hard "DO NOT RELEASE" can't be signed off — the path forward is fixing the blockers.
21. **ELECTRONICS (PCB)** — copper-survival (IPC-2221 heating, Onderdonk fusing, IR-drop / ECU brown-out) + signal integrity (diff-pair impedance, HV-aggressor coupling), reading the **same** declared peak currents as the integration ledger so "what fires at once" is never retyped.
22. **DFMEA** — the Powertrain Design Failure Mode & Effects Analysis, living in the app instead of a loose `FSAE_Powertrain_DFMEA.xlsx`. Surfaces for the Powertrain, Electrics and Cooling subteams and **opens pre-seeded** with real failure modes — the User-Guide example set (manifold crack, hose blow-off, radiator bypass, pump cavitation, motor-mount fatigue, fastener loosening, HV/LV pump-power loss) **plus active projects** (cooling test rig, gear-ratio / output-shaft sprocket, motor & diff mount, accumulator cooling) — so a new member edits real rows instead of facing a blank sheet. **RPN = S × O × D recomputes live**, with the User-Guide 1–10 Severity/Occurrence/Detection scales built in as dropdowns + an in-app reference so two people score a risk the same way; a **high-Severity row is flagged Critical even at moderate RPN**, per the guide's rule not to hide behind RPN. Three views: an editable **Risk log**, a **Dashboard** (risk-by-band donut, risk-concentration-by-subsystem bar, top-risks table, open-high-risk and open-high-severity counts), and an **Action tracker** (every open actioned row, risk-sorted — the execution list). Live **data-quality nudges** enforce the guide's definition-of-done — it flags rows marked *Closed* with no evidence and open actions with no owner. It **round-trips with the team spreadsheet**: import an existing `.xlsx`/`.csv` (column names matched automatically, RPN recomputed) and export the full log back as `FSAE_Powertrain_DFMEA.xlsx`/`.csv` in the same layout, plus a CSV of just the action tracker. Each seed row points at the KinematiK tab that produces its closure evidence (motor-mount FEA, lap-sim ratio sweep, pack-thermal, tractive safety), so a risk and the analysis that retires it sit one click apart. Edits made in the **Integration** tab feed back here automatically: a change that makes a failure more (or less) likely surfaces as a suggested Occurrence nudge on the matching row — matched by physical meaning, not exact wording, so a torque increase finds "tooth root yields under peak chain tension" on its own — with the RPN recomputed by this same engine and the human still owning the log.
23. **REGISTRY** — the component **Source of Truth**, replacing the "here's a Drive link, sorry the folder's a mess" handoff. One table shows every released part at its **current verified version**, the numbers that matter inline (the diff-mount **offset**, mass, material, units…), who signed it off and when, and a **one-click pull** of the exact released file — bytes stored locally and content-addressed by SHA-256 so the same upload is never duplicated and a row always hands back the verified file. **Ingest, don't just store:** drop the released folder as a `.zip` (or a single CAD file) and KinematiK parses real provenance out of the files themselves — model name, units, CAD system + version, author and export timestamp from the IGES Global section, and the **FEA mesh statistics** (nodes / elements / DOF / solve time) and study type out of any SOLIDWORKS Simulation `.LOG`/`.MFC` sidecars — so a deliverable advertises what it actually is instead of being one more opaque blob. Each component keeps a full **version history**; promoting a new revision is one click that timestamps the change and demotes the previous golden record to *superseded* (never deleted — handover memory). A lead **signs off** a version by name, which is timestamped onto the row. The whole registry persists to `registry.json` (blobs in `registry_blobs/`) so it commits to the repo and travels with the car, and exports as JSON. Teams that don't want to commit binaries can store just the link instead — the dashboard treats a verified link and a stored blob the same way.
