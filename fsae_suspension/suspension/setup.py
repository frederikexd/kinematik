...
PARAM_KNOBS = {
    ...
    "cg_height":         dict(label="CG height", unit="mm", step=10.0,
                              lo=240.0, hi=360.0),
    "roll_stiffness_front": dict(label="Front roll stiffness", unit="N·m/°",
                                 step=25.0, lo=150.0, hi=600.0),
    "roll_stiffness_rear":  dict(label="Rear roll stiffness", unit="N·m/°",
                                 step=25.0, lo=150.0, hi=600.0),
...
def _prepare(params: VehicleParams, front_kin, rear_kin):
    ...
    can treat camber as a free lever. Returns a copy — never mutates the caller's.
    """
    p = replace(params)
