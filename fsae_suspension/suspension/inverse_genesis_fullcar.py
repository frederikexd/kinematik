...
Session keys used:
    ...
    fullcar_result the last synthesis, held across reruns. Streamlit replays
                   the script on every widget event, so a result computed
                   inside `if st.button(...)` lives for one pass only — and
                   the buttons under it (the intent hand-off, the exports)
                   are widget events, which would throw away a search that
...
_CACHE_KEY = "fullcar_result"


def _stage_intent(targets):
    """Stage the winner's kinematic intent for the corner tab.

    A button callback, so it lands before the rerun the click triggers and
    the synthesis panel is redrawn from the cache in the same pass.
    """
    import streamlit as st
    st.session_state["genesis_targets"] = targets
...
def render():
    ...
    if st.button("🧬🏁 Synthesize the full car", type="primary", key="fc_go"):
        with st.spinner("Walking the design chain backwards…"):
            try:
                ...
                return
        # Hold the search, and the space/rules it answered, so the panel
        # below survives the widget events under it.
        ss[_CACHE_KEY] = {"res": res, "space": space, "rules": rules}

    cache = ss.get(_CACHE_KEY)
    if not isinstance(cache, dict):
        return
    res, space, rules = cache["res"], cache["space"], cache["rules"]
        ...
        with tabs[0]:
            ...
            try:
                ...
                st.dataframe(pd.DataFrame(irows), hide_index=True,
                             use_container_width=True)
                st.button("Send this intent to the InverseGenesis tab",
                          key="fc_send_intent", on_click=_stage_intent,
                          args=(tg,))
                if ss.get("genesis_targets") is not None:
