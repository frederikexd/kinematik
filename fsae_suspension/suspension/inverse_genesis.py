...
Session keys used:
    hp             read/write: the live geometry in the Kinematics tab's own
                   dialect (a coordinate dict). This is the editor's state,
                   so it is what "live" actually means.
    hardpoints     read/write: the same geometry as a Hardpoints object, the
                   dialect the other ui/ tabs read. "Apply" writes BOTH so
                   every tab consumes the generated car.
    ig_result      the last generated run, held across reruns. Streamlit
                   replays the whole script on every widget event, so a
                   result computed inside `if st.button(...)` exists for one
                   pass only — and the "apply" button, being a widget event,
                   would erase the very result it was meant to apply. The
                   run is cached here and the panel is drawn from the cache.
...
_CACHE_KEY = "ig_result"


def _hardpoints_from_session(ss):
    """The live hardpoint set if the app has one, else the default corner.

    Two dialects are accepted, `hp` first: that is the Kinematics tab's own
    coordinate dict and therefore the state the editor writes on every
    keystroke, so reading it first means a hand edit over there is never
    shadowed by an older Hardpoints object left in `hardpoints` here.
    (Ride-height PRESETS are applied downstream of `hp` in the shell and are
    not seen here — a non-default preset moves the nominal this tab seeds
    from.)
    """
    from suspension.kinematics import Hardpoints
    for key in ("hp", "hardpoints"):
        raw = ss.get(key)
        if isinstance(raw, Hardpoints):
            return raw, "live hardpoints from the Kinematics tab"
        if isinstance(raw, dict) and raw:
            try:
                return (Hardpoints.from_dict(raw),
                        "live hardpoints from the Kinematics tab")
...
_ENGINE_API = ("curves_of", "TargetCurve", "GenesisTargets", "KeepOutBox",
               "LegalVolume", "DESIGNABLE_POINTS", "CHANNELS",
               "inverse_genesis", "render_genesis_md")


def _engine_gap(ig):
    """Names this tab needs that the loaded engine module does not provide."""
    return [n for n in _ENGINE_API if not hasattr(ig, n)]


def _rounded(v, nd=6):
    """A numeric sequence as a comparable tuple."""
    import numpy as np
    return tuple(np.round(np.asarray(v, float).ravel(), nd).tolist())


def _hp_signature(hp):
    """Comparable fingerprint of a Hardpoints set."""
    out = []
    for k, v in sorted(hp.as_dict().items()):
        if isinstance(v, (list, tuple)):
            out.append((k, _rounded(v)))
...
def _declaration_signature(hp, targets, movable, half, keep_out, probe_mm,
                           clearance_mm, shop_key, pull_mm, pull_axis,
                           n_starts, n_yield, n_verify):
    """Everything the engine consumes, as one comparable value.

    Lets the cached result on screen say honestly whether it still answers
    the declarations now in the widgets. Pure — no streamlit, no state.
    """
    return (
        _hp_signature(hp),
        tuple((c.channel, _rounded(c.travel_mm), _rounded(c.target),
               _rounded(c.band)) for c in targets.curves),
        tuple(movable),
        tuple(sorted((k, round(float(v), 4)) for k, v in half.items())),
        tuple((b.label, _rounded(b.lo), _rounded(b.hi)) for b in keep_out),
        round(float(probe_mm), 4), round(float(clearance_mm), 4),
        shop_key, round(float(pull_mm), 4), str(pull_axis),
        int(n_starts), int(n_yield), int(n_verify),
    )


def _apply_to_live(winner_hp):
    """Write the generated corner into both live-geometry dialects.

    Runs as a button CALLBACK, i.e. before the rerun the click triggers,
    so the generated result is still cached and redrawn in the same pass
    instead of the panel disappearing under the user.
    """
    import streamlit as st
    ss = st.session_state
    d = winner_hp.as_dict()
    ss["hardpoints"] = winner_hp        # the ui/ package dialect
    ss["hp"] = d                        # the Kinematics editor's dialect
    # The coordinate editor's number_inputs own their widget keys; stale
    # display values there would overwrite hp on the next draw, so drop
    # them and let the editor re-read (the same move the OptimumK import
    # makes in the shell).
    for k, v in d.items():
        if isinstance(v, (list, tuple)) and len(v) == 3:
            for ax in "xyz":
                ss.pop(f"{k}_{ax}", None)
    ss.pop("_sweep_cache", None)
    ss.pop("_sweep_sig", None)
    cache = ss.get(_CACHE_KEY)
...
def render():
    ...
    gap = _engine_gap(ig)
    if gap:
        st.error("The InverseGenesis engine loaded into this process is "
                 f"missing {', '.join(gap)} — this tab cannot run against "
                 "it.")
        st.caption(
            f"Loaded from `{getattr(ig, '__file__', 'unknown')}`. Almost "
            "always a stale import rather than a missing feature: Streamlit "
            "re-executes the main script on every rerun but never re-imports "
            "a module already in sys.modules, so an engine edited after the "
            "server started keeps serving its old self. Stop and restart "
            "`streamlit run`; if that alone does not clear it, delete "
            "`suspension/__pycache__` and check the path above is the file "
    ...
    sig = _declaration_signature(hp, targets, movable, half, keep_out,
                                 probe, min_cl, _SHOPS[shop_label], pull,
                                 pull_axis, n_starts, n_yield, n_verify)

    if st.button("🧬 Generate the geometry", key="ig_run", type="primary"):
        with st.spinner("Reverse gradients pulling the points into the "
                        "curves, co-optimizing against the error field, "
                        "pricing the linearisation…"):
            try:
                ...
                return
        # Hold the run — and the declarations it answers — so the panel
        # below survives every later widget event, "apply" included.
        ss[_CACHE_KEY] = {"res": res, "targets": targets, "hp": hp,
                          "travel": float(travel), "sig": sig,
                          "applied": False}
    ...
    if cache.get("applied"):
        st.success("Applied — the live hardpoints are now the generated "
                   "corner, and every tab consumes it. Take it to Ghost "
                   "Topology next: this engine solved the rigid car; the "
                   "loaded one still needs its audit.")
    if cache.get("sig") != sig:
        st.warning(
            "The declarations have changed since this geometry was "
            "generated — what follows is the last generated result, not an "
            "answer to the panel above. Generate again to refresh."
            + (" (Applying reseeds the target curves from the generated "
               "car, which is why they now differ.)"
               if cache.get("applied") else ""))
    ...
    if res.ok:
        st.button("Apply the generated geometry to the live hardpoints",
                  key="ig_apply", on_click=_apply_to_live,
                  args=(res.winner_hp,),
                  help="Writes these coordinates into the Kinematics "
                       "editor and every tab that reads it. The generated "
