...
capped at lead/member — never owner, errors surfaced not raised into the UI,
dead links
...
def test_create_invite_allows_lead():
    client = _StubClient({"create_workspace_invite": TOK})
    tok = _auth_with(client).create_invite(_session(), WS, role="lead",
                                           ttl_hours=72, max_uses=10)
    assert tok == TOK
    name, params = client.calls[0]
    assert name == "create_workspace_invite"
    assert params["invite_role"] == "lead"


def test_create_invite_refuses_privileged_roles_client_side():
    client = _StubClient()
    for bad in ("owner", "admin"):     # lead is now permitted; owner never is
