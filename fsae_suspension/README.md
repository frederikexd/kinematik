...
Twenty-one tabs, front to back. Each takes the live geometry + setup and answers one question; nothing is retyped between them — mass/CG, peak currents, ride heights, downforce and the tire all flow from one source of truth (the **Integration** ledger), so declaring a number once propagates everywhere.
