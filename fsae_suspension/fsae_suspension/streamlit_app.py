...
with tab7:
    ...
    d_tags = st.text_input("Tags (comma-separated — e.g. roll-centre, front-upright, cooling)",
                           key="d_tags",
                           placeholder="helps next year find this later")
    if st.button("+ Log decision"):
        if d_title and d_rationale:
            ...
            for k in ("d_title", "d_tags", "d_rationale"):
    ...
    if store.decisions:
        ...
        sc = st.columns([2.4, 1.3, 1.3])
        d_query = sc[0].text_input("Search", key="dec_search",
                                   placeholder="search title, rationale, author, tags…",
                                   label_visibility="collapsed")
