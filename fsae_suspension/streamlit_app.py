  ...
  if st.session_state.get("model3d_view", "Subsystems") == "Full car":
    ...
    with _car_slot:
      try:
          ...
          # itself stays the focus. Open it when you want the detail. This whole
          # block is best-effort: if the influence summary can't be built for any
          # reason, the 3D model still renders — we just skip the read-out rather
          # than failing the entire full-car view.
          try:
