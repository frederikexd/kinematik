...
# So an idle team polls a trivial query every 2s and pays the real read cost
# just once, right after some lead posts — giving ~2s perceived delivery without
# hammering storage.
NOTE_POLL_SECONDS = 2
...
def _peek_notes_version():
    ...
    rather than assume nothing changed. Never raises."""
    try:
        return project_mod.ProjectStore(PROJECT_PATH).read_version()
    except Exception:
        return None
...
def _load_notes_from_disk():
    ...
    This is the EXPENSIVE path (full project construction); poll_note_notifications
    gates it behind the cheap version probe so it only runs when something changed."""
    try:
...
def poll_note_notifications():
    ...
    Cheap-poll optimisation: before doing the expensive full project read, ask
    the backend only for its version tag. If it matches the version we saw on the
    last tick, nothing has changed since — skip the full read entirely. This is
    what lets NOTE_POLL_SECONDS stay small without hammering storage. We fall
    through to a full read whenever the probe is None (backend can't tell) or the
    version moved, so we can never MISS a note by trusting a stale probe.
    """
    ...
    if (_ver is not None and _baseline_done and _ver == _last_ver):
        # Unchanged since last tick — the whole point of the cheap probe.
        return
    # Record the version we're about to read at, so the next tick can short-circuit.
...
with tab4:
    _3d_view = st.radio(
        "View", ["Full car", "Heatmap"], horizontal=True,
        key="model3d_view", label_visibility="collapsed",
        help="Full car assembles every subsystem into the whole "
...
with tab_car:
  if st.session_state.get("model3d_view", "Full car") == "Full car":
    st.markdown(
...
with tab_car:
  if st.session_state.get("model3d_view", "Full car") == "Heatmap":
...
with tab8:
    ...
    # We just did a full authoritative read here; adopt its version so the
    # background poller doesn't immediately repeat the expensive read.
    ...
    if st.button("Post note", key="n_post"):
        if n_msg.strip():
            ...
            # We just wrote, so the store version has moved. Adopt the new version
            # as our own high-water mark so THIS session's poller short-circuits on
            # its next tick instead of doing a redundant full read to re-discover
            # a note we already know about. Other sessions still have the old
            # version cached, so their probe will see the change and toast.
            st.session_state["_notes_version"] = nstore.read_version()
