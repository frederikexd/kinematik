...
def test_flat_pickups_give_zero_anti_dive():
    ...
    # The SVSA is at infinity (nan), confirming no anti-dive geometry. The
    # residual < 2% comes from contact-patch scrub in _path_slope_xz, which
    # is a real kinematic effect, not an anti-dive contribution.
    hp = Hardpoints.default()
    ...
    kin = SuspensionKinematics(hp)
    import numpy as np
    # SVSA at infinity confirms truly-parallel arms (zero anti-dive geometry).
    assert all(np.isnan(v) for v in kin._side_view_swing_arm())
    # The path-slope result must be negligible (< 2%) — scrub only, not anti-dive.
