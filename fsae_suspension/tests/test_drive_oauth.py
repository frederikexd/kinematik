...
def test_build_auth_url_generates_state_and_google_url(monkeypatch):
    # google_auth_oauthlib is the optional `drive` extra. Without it
    # oauth_available() is False and build_auth_url() correctly returns
    # (None, reason) — which is right behaviour, not a failure, so skip rather
    # than assert against a capability this environment does not have.
    pytest.importorskip(
